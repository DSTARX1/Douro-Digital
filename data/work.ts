export const workHero = {
  headline: "Products We've Built (That Actually Work)",
  subtitle:
    "Not case studies. Not consulting projects. Not 'we helped a client achieve X' claims we can't verify. Real products we built from scratch, shipped, and real people use. This is our track record.",
};

export const workIntro =
  "Most agencies show you \"case studies\" where they consulted on strategy and took credit. Or white-labeled platforms with the client's logo slapped on.\n\nWe don't do that.\n\nThese are products WE built. From scratch. Shipped to real users. Making real money.\n\nVoice Noob. Pocket Agent. Social Bro. Viral Kid.\n\nEach one solves a specific problem. Each one handles complexity without breaking. Each one runs 24/7 without babysitting.";

export const workCategories = [
  {
    name: "All",
    slug: "all",
  },
  {
    name: "Automation Systems",
    slug: "automation",
  },
  {
    name: "Customer Service",
    slug: "customer-service",
  },
  {
    name: "Content & Social",
    slug: "content",
  },
];

export const workProjects = [
  {
    id: "voice-noob",
    title: "Voice Noob",
    category: "automation",
    thumbnail: "/work/voice-noob-thumb.jpg",
    shortDescription:
      "Voice automation platform that lets non-technical users build complex voice workflows without coding. Handles customer intake, appointment booking, and support queries automatically.",
    tags: ["Voice Automation", "Customer Intake", "24/7 Support"],
    link: "/work/voice-noob",
  },
  {
    id: "pocket-agent",
    title: "Pocket Agent",
    category: "customer-service",
    thumbnail: "/work/pocket-agent-thumb.jpg",
    shortDescription:
      "Automated customer service that handles common questions instantly, escalates complex issues intelligently, and works while you sleep.",
    tags: ["Customer Support", "Automation", "Smart Escalation"],
    link: "/work/pocket-agent",
  },
  {
    id: "social-bro",
    title: "Social Bro",
    category: "content",
    thumbnail: "/work/social-bro-thumb.jpg",
    shortDescription:
      "Social media management that doesn't suck. Schedule, repurpose, and manage engagement without seven tabs open.",
    tags: ["Social Media", "Content Repurposing", "Engagement"],
    link: "/work/social-bro",
  },
  {
    id: "viral-kid",
    title: "Viral Kid",
    category: "content",
    thumbnail: "/work/viral-kid-thumb.jpg",
    shortDescription:
      "Content distribution system that takes one piece of content and gets it seen across platforms without manual posting everywhere.",
    tags: ["Distribution", "Multi-Platform", "Content Strategy"],
    link: "/work/viral-kid",
  },
];

export const workCTA = {
  headline: "Want to Build Something Like This?",
  description:
    "We build systems that work, handle complexity, and run 24/7 without babysitting. Ready to build something custom for YOUR business? Let's talk.",
  buttonText: "Let's Talk About Your Project",
  buttonLink: "/contact",
};

export const workFooter =
  "Every product here started the same way yours will. A conversation. A clear scope. A fixed timeline. Then we build it. Four people who give a damn. Simple as that.";
